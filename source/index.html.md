...
|typeList|int[]|否|1.限价开仓 2.市价开仓 3.限价全平 4.市价全平 5.限价部分平仓单 6.市价部分平仓单, Request Body|
|side|int|否|0:no limit 1 for sell, 2 for buy, Request Body|
|startTime|int|否|开始时间戳(单位秒), Request Body|
|endTime|int|否|结束时间戳(单位秒), Request Body|
|page|int|否|第几页,默认1, Request Query|
|limit|int|否|每页条数,默认10, Request Query|
...
|direct|int|否|1:多仓 2:空仓,0:查询所有|
|side|int|否|方向1:sell 2:buy,0:查询所有|
|startTime|number|否|0:查询所有,开始时间戳(单位微秒)|
|endTime|number|否|0:查询所有,结束时间戳(单位微秒)|
...
|Date|String|是|当前的GMT时间, Request Header|
|Digest|String|是|请求包体摘要, Request Header|
|contractCodeList|String[]|否|合约列表,为空查询所有|
|side|int|否|0:no limit 1 for sell, 2 for buy.|
